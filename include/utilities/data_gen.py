"""This module generates realistic sales data for a retail business.
...
class RealisticSalesDataGenerator:
    ...
    def __init__(self, start_date: str = "2022-01-01", end_date: str = "2023-12-31", seed: int = 123) -> None:
        self.start_date = pd.to_datetime(start_date)
        self.end_date = pd.to_datetime(end_date)
        self.us_holidays = holidays.US()  # type: ignore
    ...
    def get_seasonality_factor(self, date: pd.Timestamp, seasonality_type: str) -> float:
        ...
        if seasonality_type == "back_to_school":
            # Peak August-September
            if date.month in [8, 9]:
                return 1.4
            return 0.9

        if seasonality_type == "fitness":
    ...
    def generate_sales_data(self, output_dir: str = "/tmp/sales_data") -> dict[str, list[str]]:
        ...
        # Track file paths
        file_paths = {"sales": [], "inventory": [], "customer_traffic": [], "promotions": [], "store_events": []}
        ...
        while current_date <= self.end_date:
            ...
            for store_id, store_info in self.stores.items():
                ...
                weather_factor = self.rng.normal(1.0, 0.1)
                ...
                if not store_events_df.empty:
                    event = store_events_df[
                        (store_events_df["store_id"] == store_id) & (store_events_df["date"] == current_date)
                    ]
                    if not event.empty:
                ...
                for product_id, product_info in self.all_products.items():
                    ...
                    if not promotions_df.empty:
                        promo = promotions_df[
                            (promotions_df["date"] == current_date) & (promotions_df["product_id"] == product_id)
                        ]
                        if not promo.empty:
                    ...
                    quantity = int(base_quantity * seasonality_factor * promotion_factor * self.rng.normal(1.0, 0.2))
