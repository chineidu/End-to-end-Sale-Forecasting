...
class ModelVisualizer(ABC):
    ...
    @abstractmethod
    def create_performance_dashboard(self, metrics_dict: dict[str, dict[str, float]], save_path: str | None = None) -> Any:
...
class ModelVisualizerMatMatplotlib(ModelVisualizer):
    ...
    def create_feature_importance_comparison(
        ...
        for idx, (model_name, importance_df) in enumerate(feature_importance_dict.items()):
            ...
            _ = ax.barh(y_pos, top_features["importance"], color=self.colors.get(model_name.lower(), "#95A5A6"), alpha=0.7)
...
class ModelVisualizerPlotly(ModelVisualizer):
    ...
    def create_performance_dashboard(
        ...
        fig = make_subplots(
            rows=3,
            cols=3,
            subplot_titles=["RMSE", "MAE", "MAPE (%)", "R² Score", "Overall Model Ranking", "Normalized Performance"],
            specs=[
        ...
        for metric, title, lower_better, row, col in metrics_info:
            ...
            fig.add_trace(
                go.Bar(
                    x=models,
                    y=values,
                    marker={"color": colors, "line": {"color": edge_colors, "width": edge_widths}},
    ...
    def create_prediction_quality_analysis(
        ...
        predictions_dict = {k: (v.to_pandas() if isinstance(v, pl.DataFrame) else v) for k, v in predictions_dict.items()}
        ...
        for idx, (model_name, pred_df) in enumerate(predictions_dict.items()):
            ...
            if "prediction_lower" in pred_df.columns and "prediction_upper" in pred_df.columns:
                plot_data_conf = pd.merge(
                    plot_data, pred_df[[date_col, "prediction_lower", "prediction_upper"]], on=date_col, how="left"
                )
        ...
        # Update layout
        fig.update_layout(height=600, title_text="Prediction Quality Analysis", title_x=0.5, showlegend=True)
    ...
    def create_residuals_diagnostic_panel(
        ...
        # Update layout
        fig.update_layout(height=800, title_text="Residuals Diagnostic Panel", title_x=0.5, showlegend=True)
    ...
    def create_feature_importance_comparison(
        ...
        # Update layout
        fig.update_layout(
            height=900, title_text=f"Feature Importance Analysis - Top {top_n} Features", title_x=0.5, showlegend=False
        )
    ...
    def create_model_stability_analysis(
            ...
            for model_name, data in stability_data.items():
                fig.add_trace(
                    go.Scatter(
                        ...
                        y=data[metric],
                        mode="lines",
                        line={"color": self.colors.get(model_name.lower(), "#95A5A6"), "width": 2},
                        name=model_name,
        ...
        # Update layout
        fig.update_layout(
            height=700, title_text=f"Model Stability Analysis (Rolling Window: {window_size})", title_x=0.5, showlegend=True
        )
...
def generate_model_comparison_report(
    run_id: str, test_data: pd.DataFrame | pl.DataFrame, viz_backend: Literal["matplotlib", "plotly"]
) -> dict[str, str]:
